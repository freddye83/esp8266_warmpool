...
bool valid_msg() {
 ...
 // if  ((msg.size() < 193) and (msg[0]=0x01)) return false;
 /* if  ((msg.size() == 189) and (msg[0]=0x00)) return true;
  if  ((msg.size() == 193) and (msg[0]=0x01)) return true;
  return false;*/
...
void sendmsg(uint8_t * message, uint8_t len) {
  ...
  digitalWrite(WARMPOOL_RTS, HIGH);
